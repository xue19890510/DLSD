...
class BaselineTrainAdv(nn.Module):
    def __init__(self, params, model_func, num_class):
        ...
        if params.method in ['stl_deepbdc', 'meta_deepbdc', 'protonet', 'good_embed']:
            ...
            self.grl_layer = WarmStartGradientReverseLayer(alpha=1.0, lo=0.0, hi=0.1, max_iters=170, auto_step=False).cuda()
            self.adv_head =  AddMarginProduct(self.feat_dim, num_class)
