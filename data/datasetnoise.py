...
class SimpleDatasetCSV:
    ...
    def __getitem__(self, i):
        ...
        img_orgin = Image.open(image_path).convert('RGB')
        img = self.transform(img_orgin)
        img_noise = self.transform(img_orgin)
        target = self.target_transform(self.label[i] - min(self.label))
        return img,img_noise, target
...
class SetDatasetCSV:
    def __init__(self, data_path, data_file_list, batch_size, transform):
        ...
        sub_data_loader_params = dict(batch_size=batch_size,
                                      ...
                                      pin_memory=False)
        for cl in self.cl_list:
            sub_dataset = SubDatasetCSV(self.sub_meta[cl], cl, transform=transform)                        #为每一个class 创建一个dataloader
            self.sub_dataloader.append(torch.utils.data.DataLoader(sub_dataset, **sub_data_loader_params))

    def __getitem__(self, i):
        a=next(iter(self.sub_dataloader[i]))
        return a
