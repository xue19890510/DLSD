...
def ssl_collate_fn_noise(batch):
    input, input_noise,target = zip(*batch)
    input = torch.stack(input)
    input_noise = torch.stack(input_noise)
    ...
    input_noise = create_4rotations_images(input_noise)

    return input,input_noise, (target, rot_target)
